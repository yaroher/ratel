...
// MergedFS Implements the fs.FS interface, using the two underlying FS's. If a file is
...
// MergedDirectory This is the key component of this library. It represents a directory that is
...
func (d *MergedDirectory) Read(_ []byte) (int, error) {
	return 0, fmt.Errorf("%s is a directory", d.name) //nolint:err113 // err
}
...
func getMergedDirEntry(aDir, bDir fs.DirEntry) (fs.DirEntry, error) {
	infoA, e := aDir.Info()
	if e != nil {
...
func mergeDirEntries(aFile, bFile fs.File) ([]fs.DirEntry, error) { //nolint:funlen,cyclop // This is a big function.
	dirA, aOk := aFile.(fs.ReadDirFile)
	if !aOk {
		return nil, ErrImplementReadDirFile
	}

	dirB, bOk := bFile.(fs.ReadDirFile)
	if !bOk {
		return nil, ErrImplementReadDirFile
	}
	...
	for _, entry := range entriesA {
		...
		if conflicts {
			...
			return nil, fmt.Errorf("duplicate name in dir A: %s", name) //nolint:err113 // err
		}
	...
	for _, entry := range entriesB {
		...
		mergedDirEntry, err := getMergedDirEntry(existingEntry, entry)
		if err != nil {
...
func (m *MergedFS) newMergedDirectory(aFile, bFile fs.File, path string) (fs.File, error) {
	defer aFile.Close()
	defer bFile.Close()
	...
	modTime := statA.ModTime().Unix()
	modTimeB := statB.ModTime().Unix()

	if modTimeB > modTime {
		modTime = modTimeB
	}

	entries, e := mergeDirEntries(aFile, bFile)
	if e != nil {
...
func (m *MergedFS) validatePathPrefix(path string) error {
	...
	for i := range components {
		...
		file, err := m.A.Open(prefix)
		if err != nil {
			if isBadPathError(err) {
			...
			return fmt.Errorf("%w: Error opening %s in A: %w", fs.ErrNotExist, path, err)
		}
		...
		if err != nil {
			return fmt.Errorf("couldn't stat file in A: %w", err)
		}
...
func (m *MergedFS) UsePathCaching(enabled bool) {
	...
	nestedMergedFS, aOk := m.A.(*MergedFS)
	if aOk {
		nestedMergedFS.UsePathCaching(enabled)
	}

	nestedMergedFS, bOk := m.B.(*MergedFS)
	if bOk {
		nestedMergedFS.UsePathCaching(enabled)
	}
}

// Open If the path corresponds to a directory present in both A and B, this returns
...
func (m *MergedFS) Open(path string) (fs.File, error) { //nolint:funlen,gocognit,cyclop // This is a big function.
	if !fs.ValidPath(path) {
		return nil, &fs.PathError{Op: "open", Path: "path", Err: fs.ErrInvalid}
	}

	fileAByte, e := m.A.Open(path)
	if e == nil { //nolint:nestif // This is the most common case
		fileInfo, statErr := fileAByte.Stat()
		if statErr != nil {
			fileAByte.Close()

			return nil, fmt.Errorf("couldn't stat %s in FS A: %w", path, statErr)
		}

		if !fileInfo.IsDir() {
			// If the file isn't a directory, we know it always overrides FS B,
			// so we don't even need to check FS B.
			return fileAByte, nil
		}

		// The file is a directory in A, so we need to see if a directory with
		// the same name exists in B.
		fileBByte, statErr := m.B.Open(path)
		if statErr != nil {
			if isBadPathError(statErr) {
			...
			return nil, fmt.Errorf("couldn't open %s in FS B: %w", path, statErr)
		}
		// Check if the file in B is a directory.
		fileInfo, statErr = fileBByte.Stat()
		if statErr != nil {
			fileAByte.Close()
			fileBByte.Close()

			return nil, fmt.Errorf("couldn't stat %s in FS B: %w", path, statErr)
		}
		...
		return m.newMergedDirectory(fileAByte, fileBByte, path)
	}
	...
	fByte, e := m.B.Open(path)
	if e != nil {
		return nil, e //nolint:wrapcheck // std
	}
...
func (m *MergedFS) ReadFile(name string) ([]byte, error) {
	...
	return io.ReadAll(file) //nolint:wrapcheck // short
}

// EmptyFS Implements the FS interface, but provides a filesystem containing no files.
...
func balancedMergeRecursive(content []fs.FS) fs.FS {
	...
	if len(content) == 2 { //nolint:mnd // 2 is an even number
		return NewMergedFS(content[0], content[1])
	}

	lSize := len(content) / 2 //nolint:mnd // 2 is an even number
...
// MergeMultiple Merges an arbitrary list of filesystems into a single filesystem. The first
