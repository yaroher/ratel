package mfs //nolint:testpackage // We want to test file.
...
func openZip(t *testing.T, path string) fs.FS {
	t.Helper()
	...
	toReturn, e := zip.NewReader(file, stat.Size())
	if e != nil {
...
func TestMergedFS(t *testing.T) {
	...
	zip1 := openZip(t, "test_data/test_a.zip")
	zip2 := openZip(t, "test_data/test_b.zip")
	zip3 := openZip(t, "test_data/test_c.zip")
...
func TestReadFileFS(t *testing.T) {
	...
	// Make sure this is the copy from test_a.zip, which should be two bytes.
	file, err := merged.Open("test1.txt")
	if err != nil {
	...
	data, err := merged.ReadFile("test1.txt")
	if err != nil {
...
func TestPathPrefixCaching(t *testing.T) { //nolint:cyclop // This is a test.
	t.Parallel()
...
const FileMod0666 = fs.FileMode(0666) | fs.ModeDir //nolint:gofumpt // 0 is an octal number
...
func generateDeepBenchmarkingFS() (*MergedFS, []string) { //nolint:gocritic // nonamedreturns shit
	fsA := fstest.MapFS(make(map[string]*fstest.MapFile))
	fsB := fstest.MapFS(make(map[string]*fstest.MapFile))
...
func runFSBenchmark(b *testing.B, merged *MergedFS, paths []string) {
	...
	for n := 0; n < b.N; n++ {
		for _, path := range paths {
			...
			stats = nil //nolint:wastedassign // We don't care about this value.
